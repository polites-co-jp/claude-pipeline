...
GitHub Issue (auto-implement ラベル)
  ...
  ジョブをスケジューリング
       ...
       │    │    └─ テスト失敗 → 実装修正 & 再テスト（テスト通過まで無制限リトライ）
       │    │    └─ テスト未通過 → レビューせずパイプライン中断
...
**レビュー差し戻しは最大 3 回**、**テスト失敗時の実装修正はテストが通るまで無制限**にリトライします。
テストが通過していないコードはレビューしません（パイプラインを中断します）。
...
イテレーション 1:
    テストコード実装 & テスト実施
        │
        ├─ テスト通過 → レビューへ進む
        └─ テスト失敗 → 実装修正 & 再テスト（通過するまで無制限リトライ）
        │       └─ それでも通過しない → レビューせずパイプライン中断
        ▼
    レビュー（問題点を指摘、コード修正なし）
        │
...
テストが失敗した場合、実装コードを修正してテストをパスさせます（テスト通過まで無制限リトライ）。
...
| イテレーション | 動作 |
|---------------|------|
| 1 回目 | テスト実施(失敗→実装修正を無制限リトライ) → review → LGTM ならループ終了、NEEDS_FIX なら re-implement → コミット |
| 2 回目 | テスト実施(失敗→実装修正を無制限リトライ) → review → LGTM ならループ終了、NEEDS_FIX なら re-implement → コミット |
| 3 回目（最終） | テスト実施(失敗→実装修正を無制限リトライ) → review → LGTM でも NEEDS_FIX でもループ終了（最終イテレーションでは再実装しない） |

> テストが通過しない場合、レビューは実行せずパイプラインを中断します。
